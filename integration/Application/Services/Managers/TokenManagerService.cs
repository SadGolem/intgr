...
{
    Task<(string aproToken, string mtToken)> GetTokensAsync();
}
...
public class TokenManagerService : ITokenManagerService
{
    ...
    public async Task<(string aproToken, string mtToken)> GetTokensAsync()
    {
        try
        {
        ...
        catch (ApiAuthException ex)
        {
            ...
            throw; // Можно создать кастомное исключение
        }
        catch (Exception ex)
        {
